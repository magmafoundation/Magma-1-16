...
public class CraftLivingEntity extends CraftEntity implements LivingEntity {
    ...
    public int getArrowCooldown() {
        return getHandle().removeArrowTime; // PAIL rename arrowCooldown
    }
    ...
    public boolean addPotionEffect(PotionEffect effect, boolean force) {
        getHandle().addEffect(new EffectInstance(Effect.byId(effect.getType().getId()), effect.getDuration(), effect.getAmplifier(), effect.isAmbient(), effect.hasParticles()), EntityPotionEffectEvent.Cause.PLUGIN);
        return true;
    }
    ...
    public <T extends Projectile> T launchProjectile(Class<? extends T> projectile, Vector velocity) {
        ...
        if (Snowball.class.isAssignableFrom(projectile)) {
            ...
            ((ThrowableEntity) launch).shootFromRotation(getHandle(), getHandle().yRot, getHandle().xRot, 0.0F, 1.5F, 1.0F); // ItemSnowball
        } else if (Egg.class.isAssignableFrom(projectile)) {
            launch = new EggEntity(world, getHandle());
            ((ThrowableEntity) launch).shootFromRotation(getHandle(), getHandle().yRot, getHandle().xRot, 0.0F, 1.5F, 1.0F); // ItemEgg
        } else if (EnderPearl.class.isAssignableFrom(projectile)) {
            launch = new EnderPearlEntity(world, getHandle());
            ((ThrowableEntity) launch).shootFromRotation(getHandle(), getHandle().yRot, getHandle().xRot, 0.0F, 1.5F, 1.0F); // EnderPearlItem
            ...
            ((AbstractArrowEntity) launch).shootFromRotation(getHandle(), getHandle().yRot, getHandle().xRot, 0.0F, 3.0F, 1.0F); // ItemBow
            ...
            ((ThrowableEntity) launch).shootFromRotation(getHandle(), getHandle().yRot, getHandle().xRot, -20.0F, 0.5F, 1.0F); // SplashPotionItem
        } else if (ThrownExpBottle.class.isAssignableFrom(projectile)) {
            launch = new ExperienceBottleEntity(world, getHandle());
            ((ThrowableEntity) launch).shootFromRotation(getHandle(), getHandle().yRot, getHandle().xRot, -20.0F, 0.7F, 1.0F); // ExperienceBottleItem
